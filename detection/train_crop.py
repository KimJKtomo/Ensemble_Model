...
# =========================
# Online GT-cropping (append) for TRAIN ONLY
# =========================
def online_gtcrop_append(imgs, targets, p_select=0.7, k=1,
                         context_scale=1.4, jitter=0.1, square=True,
                         min_keep_iou=0.2, keep_others=True):
    ...
    B, _, H, W = imgs.shape
    bidx = targets[:, 0].long()
    per_img = [torch.where(bidx == bi)[0] for bi in range(B)]
    add_imgs, add_tgts = [], []
    for bi in range(B):
        ids = per_img[bi]
        if ids.numel() == 0: continue
        if torch.rand(1, device=device).item() > p_select: continue
        for _ in range(k):
            ii = ids[torch.randint(len(ids), (1,), device=device)].item()
            _, cj0, cx0, cy0, w0, h0 = targets[ii]
            cxp, cyp = float(cx0)*W, float(cy0)*H
            bw, bh = float(w0)*W, float(h0)*H
            cw = bw * context_scale; ch = bh * context_scale
            cxp += (torch.rand(1, device=device).item()*2-1) * jitter * bw
            cyp += (torch.rand(1, device=device).item()*2-1) * jitter * bh
            if square:
                side = max(cw, ch); cw = ch = side
            x1 = max(0.0, cxp - cw/2); y1 = max(0.0, cyp - ch/2)
            x2 = min(float(W), cxp + cw/2); y2 = min(float(H), cyp + (cw/2 if square else ch/2))
            if x2-x1 < 2 or y2-y1 < 2: continue
            ix1, iy1, ix2, iy2 = map(lambda z: int(round(float(z))), (x1,y1,x2,y2))
            crop = imgs[bi:bi+1, :, iy1:iy2, ix1:ix2]
            crop = F.interpolate(crop, size=(H, W), mode='bilinear', align_corners=False)
            add_imgs.append(crop)
            keep = []
            sx, sy = W/(x2-x1+1e-9), H/(y2-y1+1e-9)
            for j in ids.tolist():
                _, cj, cxj, cyj, wj, hj = targets[j]
                bx, by = float(cxj)*W, float(cyj)*H
                bwj, bhj = float(wj)*W, float(hj)*H
                bx1, by1 = bx-bwj/2, by-bhj/2
                bx2, by2 = bx+bwj/2, by+bhj/2
                ix1b, iy1b = max(bx1,x1), max(by1,y1)
                ix2b, iy2b = min(bx2,x2), min(by2,y2)
                iw, ih = max(0.0, ix2b-ix1b), max(0.0, iy2b-iy1b)
                inter = iw*ih
                ab = max(0.0,(bx2-bx1)*(by2-by1)); ac = max(1.0,(x2-x1)*(y2-y1))
                denom = ab + ac - inter
                iou = inter/denom if denom>0 else 0.0
                if (j==ii) or (keep_others and iou>=min_keep_iou):
                    nx1, ny1 = max(0.0, bx1-x1), max(0.0, by1-y1)
                    nx2, ny2 = max(0.0, bx2-x1), max(0.0, by2-y1)
                    rx1, ry1, rx2, ry2 = nx1*sx, ny1*sy, nx2*sx, ny2*sy
                    rw, rh = max(1.0, rx2-rx1), max(1.0, ry2-ry1)
                    rcx, rcy = rx1+rw/2, ry1+rh/2
                    keep.append(torch.tensor([0, cj, rcx/W, rcy/H, rw/W, rh/H], device=device))
            if len(keep):
                new_bi = B + len(add_imgs) - 1
                k_t = torch.stack(keep, dim=0); k_t[:,0] = new_bi
                add_tgts.append(k_t)
    if len(add_imgs)==0: return imgs, targets
    return torch.cat([imgs]+add_imgs, dim=0), torch.cat([targets]+add_tgts, dim=0)
...
def _flatten_one_level(x):
    ...
    for e in x:
        if isinstance(e, (list, tuple)):
            for t in e:
                if torch.is_tensor(t): out.append(t)
        else:
            out.append(e)
    return out
...
def train(hyp, opt, device, callbacks):
    ...
    data_dict = None
    if RANK in {-1, 0}:
        if not hasattr(opt, 'upload_dataset'):
    ...
    # --- Model build + safe backbone-only weight load ---
    ...
    if pretrained:
        ...
        msd = intersect_dicts(csd, model.state_dict(), exclude=exclude)
        model.load_state_dict(msd, strict=False)
        LOGGER.info(f'Transferred {len(msd)}/{len(model.state_dict())} items from {weights}')
    else:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            # Append GT-centered crops
            imgs, targets = online_gtcrop_append(
                imgs, targets,
                p_select=0.7,
                k=1,
                context_scale=1.4,
            ...
            with torch.amp.autocast("cuda", enabled=amp):
                ...
                if isinstance(pred, (list, tuple)) and len(pred) == 2:
                    feats, scores = pred
                    feats = _flatten_one_level(feats)
                    scores = _flatten_one_level(scores)
                    pred = (feats, scores)
                ...
                if isinstance(pred, (list, tuple)) and len(pred) and isinstance(pred[0], (list, tuple)):
                    norm = []
                    for y in pred:
                        if isinstance(y, (list, tuple)) and len(y) and all(isinstance(t, torch.Tensor) for t in y):
                            # pred_distri, pred_scores는 공간축 동일해야 함
                            norm.append(torch.cat(y, dim=1))
                ...
                loss, loss_items = ComputeLoss(model)(tal_in, targets)
                if RANK != -1:
                    loss *= WORLD_SIZE
                if opt.quad:
                    loss *= 4.
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                if f is last:
                    strip_optimizer(f, w / 'last_striped.pt')
                else:
                    strip_optimizer(f, w / 'best_striped.pt')
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/meta_mix.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-high.yaml', help='hyperparameters path')
    parser.add_argument('--epochs', type=int, default=100, help='total training epochs')
    parser.add_argument('--batch-size', type=int, default=8, help='total batch size for all GPUs, -1 for autobatch')
    parser.add_argument('--imgsz', type=int, default=832, help='train, val image size (pixels)')
    ...
    parser.add_argument('--evolve', type=int, nargs='?', const=300, help='evolve hyperparameters')
    parser.add_argument('--bucket', type=str, default='', help='gsutil bucket')
    parser.add_argument('--cache', type=str, nargs='?', const='ram', help='image --cache ram/disk')
    parser.add_argument('--image-weights', action='store_true', help='weighted image selection')
    ...
    parser.add_argument('--local_rank', type=int, default=-1, help='DDP arg')
